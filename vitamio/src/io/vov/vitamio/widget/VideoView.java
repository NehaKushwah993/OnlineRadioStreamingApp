...
public class VideoView extends SurfaceView implements MediaController.MediaPlayerControl {
	...
	OnVideoSizeChangedListener mSizeChangedListener = new OnVideoSizeChangedListener() {
      ...
      Lg.d("onVideoSizeChanged: (%dx%d)", width, height);
  ...
  private OnErrorListener mErrorListener = new OnErrorListener() {
    public boolean onError(MediaPlayer mp, int framework_err, int impl_err) {
      Lg.d("Error: %d, %d", framework_err, impl_err);
  ...
  private OnInfoListener mInfoListener = new OnInfoListener() {
    ...
    public boolean onInfo(MediaPlayer mp, int what, int extra) {
      Lg.d("onInfo: (%d, %d)", what, extra);
    	
      	if(MediaPlayer.MEDIA_INFO_UNKNOW_TYPE == what){
    		Lg.e(" VITAMIO--TYPE_CHECK  stype  not include  onInfo mediaplayer unknow type ");
    	} 
  ...
  private OnTimedTextListener mTimedTextListener = new OnTimedTextListener() {
    ...
    public void onTimedTextUpdate(byte[] pixels, int width, int height) {
      Lg.i("onSubtitleUpdate: bitmap subtitle, %dx%d", width, height);
  ...
  protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
	...
	public void setVideoLayout(int layout, float aspectRatio) {
        ...
        if(mVideoAspectRatio <= 1.0 && mVideoAspectRatio!= 0.0){
            if(mContext.getResources().getConfiguration().orientation == Configuration.ORIENTATION_PORTRAIT){
                layout = VIDEO_LAYOUT_FIT_PARENT;
            }else{
                layout = VIDEO_LAYOUT_ORIGIN;
            }
...
//		} else
    ...
    Lg.d("VIDEO: %dx%dx%f, Surface: %dx%d, LP: %dx%d, Window: %dx%dx%f", mVideoWidth, mVideoHeight, mVideoAspectRatio, mSurfaceWidth, mSurfaceHeight, lp.width, lp.height, windowWidth, windowHeight, windowRatio);
    mVideoLayout = layout;
...
//      return;
    ...
    mContext.sendBroadcast(i);
		...
		try {
			...
			Lg.e("Unable to open content: " + mUri, ex);
			mCurrentState = STATE_ERROR;
			mTargetState = STATE_ERROR;
  ...
  public void suspend() {
    if (isInPlaybackState()) {
      ...
      Lg.d("Unable to suspend video. Release MediaPlayer.");
    }
  }
