...
public class MediaPlayer {
  ...
  public MediaPlayer(Context ctx, boolean preferHWDecoder) {
    ...
    if (preferHWDecoder) {
      ...
      try {
        ...
        Lg.e("unloadOMX failed %s", e.toString());
      }
  ...
  public void start() throws IllegalStateException {
    stayAwake(true);
    if (mInBuffering) {
        //Lg.i("MiuiVideo: now is in buffering, and will start after buffering");
        mNeedResume = true;
    } else {
        //Lg.i("MiuiVideo: start player");
        _start();
    }
  }
...
private int audioTrackInit(int sampleRateInHz, int channels) {
 ...
 audioTrackRelease();
    ...
    try {
      ...
      Lg.e("audioTrackInit", e);
    }
    return mAudioTrackBufferSize;
  }
  public int audioTrackInit() {
//	  Lg.e("  ffff mediaplayer audiotrackinit start .  sampleRateInHz:=" + sampleRateInHz + " channels:=" + channels );
	    audioTrackRelease();
	    int channelConfig = channels >= 2 ? AudioFormat.CHANNEL_OUT_STEREO : AudioFormat.CHANNEL_OUT_MONO;
	    try {
	      mAudioTrackBufferSize = AudioTrack.getMinBufferSize(sampleRateInHz, channelConfig, AudioFormat.ENCODING_PCM_16BIT);
  ...
  private class EventHandler extends Handler {
    ...
    private void onInfo(Message msg) {
        switch (msg.arg1) {
            case MediaPlayer.MEDIA_INFO_BUFFERING_START:
                {
                    //Lg.i("MiuiVideo: receive buffer start, isplaying " + isPlaying());
                    mInBuffering = true;
                    if (isPlaying()) {
                        //Lg.i("MiuiVideo: pause player for buffer start");
                        _pause();
                        mNeedResume = true;
                    }
                    break;
            ...
            case MediaPlayer.MEDIA_INFO_BUFFERING_END:
                {
                    //Lg.i("MiuiVideo: receive buffer end, needResume = " + mNeedResume);
                    mInBuffering = false;
                    if (mNeedResume) {
                        //Lg.i("MiuiVideo: start player after buffer");
                        _start();
                        mNeedResume = false;
                    }
                    break;
    ...
    private void onBufferingUpdate(Message msg) {
        ...
        if (percent >= 100 && mInBuffering) {
            //Lg.i("MiuiVideo: receive buffer 100, needResume = " + mNeedResume);
            mInBuffering = false;
            if (mNeedResume) {
                //Lg.i("MiuiVideo: start player after buffer 100");
                _start();
                mNeedResume = false;
            }
            if (mOnInfoListener != null) {
                //Lg.i("MiuiVideo: add one buffer end event");
...
//        //Lg.i("MiuiVideo: get message after player released, msg type: " + msg.what);
        return;
      }
      switch (msg.what) {
      case MEDIA_ERROR_TIMED_OUT:
    	  ...
    	  Lg.e(" for dedao MEDIA_ERROR_TIMED_OUT (%d, %d)", msg.arg1, msg.arg2);
    	  return;
        case MEDIA_PREPARED:
        ...
        case MEDIA_SET_VIDEO_SIZE:
          ...
          return;
        case MEDIA_ERROR:
          Lg.e("Error (%d, %d)", msg.arg1, msg.arg2);
          ...
          return;
        case MEDIA_INFO:
          Lg.i("Info (%d, %d)", msg.arg1, msg.arg2);
          if (mOnInfoListener != null)
            mOnInfoListener.onInfo(mMediaPlayer, msg.arg1, msg.arg2);
          return;
        case MEDIA_CACHE:
        ...
        case MEDIA_TIMED_TEXT:
          ...
          if (mData.getInt(MEDIA_SUBTITLE_TYPE) == SUBTITLE_TEXT) {
            Lg.i("Subtitle : %s", mData.getString(MEDIA_SUBTITLE_STRING));
        ...
        case MEDIA_HW_ERROR:
        	...
        	return;
        default:
          Lg.e("Unknown message type " + msg.what);
          return;
      }
    }
  }
